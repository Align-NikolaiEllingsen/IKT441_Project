...
class GameState:
    def __init__(self, display_hidden=False):
        ...
        mui = self.create_marker(500, 525, 10, THECOLORS["yellow"])
        ...
        self.static = [
            ...
            pymunk.Segment(self.space.static_body,(muo.position), (ulo.position), 2),
            ...
            pymunk.Segment(self.space.static_body,(mui.position), (uli.position), 2),
        ...
        # Counter and score is used for different purposes, score for car, counter for placing checkpoints
        self.score = 0
        self.counter = 0

        def checkpoint_hit(space, arbiter, data): # Do this if called by collision handler
            if self.counter < 7: # If less than 7, remove current and place next checkpoint
                self.space.remove(self.checkpoint[self.counter])
                self.score += 1
                self.counter += 1
                self.space.add(self.checkpoint[self.counter])
            else: # If more than 7, remove current checkpoint and add checkpoint 0 again
                self.space.remove(self.checkpoint[self.counter])
                self.counter = 0
                self.space.add(self.checkpoint[self.counter])
            return True
    ...
    def frame_step(self, action):
        ...
        # Places the AR-19 car over the "car" object
    ...
    # Peformed when the car has crashed
    def handle_crash(self, driving_direction):
        ...
        self.car_body.position = 159, coordfix(600) # Restart coordinates
