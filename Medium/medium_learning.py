...
def train_net(model, params):
    ...
    epsilon = 0.5
    train_frames = 2000000  # Number of frames to play.
    ...
    while t < train_frames:
        ...
        if t % 500000 == 0:
            model.save_weights('medium_models/' + filename + '-' +
                               str(t) + '.h5',
                               overwrite=True)
...
def log_results(filename, data_collect, loss_log):
    ...
    with open('medium_results/learn_data-' + filename + '.csv', 'w', newline='') as data_dump:
        wr = csv.writer(data_dump)
        wr.writerows(data_collect)

    with open('medium_results/loss_data-' + filename + '.csv', 'w', newline='') as lf:
        wr = csv.writer(lf)
...
def launch_learn(params):
    ...
    if not os.path.isfile('medium_results/loss_data-' + filename + '.csv'):
        # Create file so we don't double test when we run multiple
        # instances of the script at the same time.
        open('medium_results/loss_data-' + filename + '.csv', 'a').close()
        print("Starting test.")
        # Train.
