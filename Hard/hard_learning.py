...
hardmap.fps = 9999 # Simulation speed
...
def train_net(model, params):
    ...
    epsilon = 0.5
    train_frames = 4000000  # Number of frames to play.
    ...
    game_state = hardmap.GameState(display_hidden=True) # Hide the game screen to improve performance
    ...
    while t < train_frames:
        ...
        if t % 500000 == 0:
            model.save_weights('hard_models/' + filename + '-' +
                               str(t) + '.h5',
                               overwrite=True)
...
def log_results(filename, data_collect, loss_log):
    ...
    with open('hard_results/learn_data-' + filename + '.csv', 'w', newline='') as data_dump:
        wr = csv.writer(data_dump)
        wr.writerows(data_collect)

    with open('hard_results/loss_data-' + filename + '.csv', 'w', newline='') as lf:
        wr = csv.writer(lf)
...
def launch_learn(params):
    ...
    if not os.path.isfile('hard_results/loss_data-' + filename + '.csv'):
        # Create file so we don't double test when we run multiple
        # instances of the script at the same time.
        open('hard_results/loss_data-' + filename + '.csv', 'a').close()
        print("Starting test.")
        # Train.
